...
class CombatSession:
    ...
    def resolve_turn(self):
        ...
        for name, player in self.players.items():
            ...
            # 3. Только теперь тикаем (уменьшаем длительность)
            # Если в character.py tick_effects() тоже принтит, можно временно закомментировать принт внутри него,
            # чтобы не было дублей в логе.
            player.tick_effects()
